...
class ProductMeiliSearch(WebsiteSale):
    ...
    def _prepare_meili_search_args(self):
        if not self.meili_server_address:
            self.meili_server_address = (
                request.env["ir.config_parameter"]
                .sudo()
                .get_param("msearch.service.url")
                or False
            )
            if not self.meili_server_address:
                raise ValidationError(
                    "Cant't get meilisearch server address from database."
                )

        if not self.meili_server_api_key:
            self.meili_server_api_key = (
                request.env["ir.config_parameter"]
                .sudo()
                .get_param("msearch.service.apikey")
                or False
            )

        if not self.meili_product_index:
            self.meili_product_index = (
                request.env["ir.config_parameter"]
                .sudo()
                .get_param("msearch.service.product.index")
                or "products"
            )
            if not self.meili_product_index:
                raise ValidationError(
                    "Cant't get meilisearch server products index from database."
                )
    ...
    def products_autocomplete(self, term, options={}, **kwargs):
        ...
        res = super(ProductMeiliSearch, self).products_autocomplete(
            term, options, **kwargs
        )
